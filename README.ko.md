...
현재 UI는 교목·관목 각각 연도별 추정/기여도와 직경별 추정/기여도, 시각화까지 총 9개 탭을 제공합니다. 상단 공통 연도 바가 0~30년의 저장량·기여도·3D를 함께 갱신합니다. 생장량 없는 수종에는 연 2% 가정을 적용합니다. 상세값은 [가정 생장률 기록](GROWTH_ASSUMPTIONS.md)을 참조하세요. 아래 논문 그림은 이전 UI 예시입니다.
...
| 학명 | 상대생장식 | 예측변수 | 적합범위 | 1–10 | 11–20 | 21–30 |
|---|---|:---:|:---:|---:|---:|---:|
| ***교목 컬렉션*** | | | | | | |
...
- **연도별** — 향후 30년 시나리오. **77개** 레코드 모두 표시하며, 핵심 22개는 기존 생장량을, 나머지 55개는 연 2% 가정 생장률을 적용한다.
...
확장 컬렉션 레코드도 대상지 합계, 식재면적 검증(개체당 면적은 수종별이 아니라 생장형별로 정의됨), Excel 내보내기에 포함된다. 결과 표 아래에는 핵심/확장 소계를 따로 표시해 22개 레코드 기준 수치를 그대로 확인할 수 있다. 3D 시각화도 연도별 그래프와 같은 기존/가정 생장률을 사용해 77개 레코드 모두 포함한다.
...
- 국내·국외 레코드는 `Y_i = f_i(X_i, H_i)` 형태로 AST 허용목록을 통해 평가한다(Python `eval` 미사용). 탄소계수·생장량을 저장하지 않으므로 **고정 CF₀ = 0.5** 를 적용한다. 연도별 시나리오는 X(t) = X(0) × 1.02^t를 가정하며 H는 고정한다. 직경별 그래프도 제공한다.
...
현재 소프트웨어의 그래프와 3D 시각화는 0–30년을 제공합니다. 아래 Figure 5와 표는 기존 50년 시나리오의 논문용 예시입니다.
...
- 각 표는 그 생장형의 **전체 수종**(교목 59종 · 관목 18종)을 보여준다. 계수 열(`a, b, CF`·범위·성장률)과 식 열(상대생장식·범위·변수)을 하나의 표로 합쳤고, 행에 해당하지 않는 열은 비어 있으며 수정할 수 없다. **구분** 열이 각 행을 핵심·국내·국외로 표시한다.
