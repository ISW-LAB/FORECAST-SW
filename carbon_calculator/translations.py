...
EN: dict[str, str] = {
    ...
    "직경별은 유효범위를 훑어 전체 수종을 표시합니다.":
        "The year basis plots only species with annual diameter increments. The "
        "diameter basis sweeps the fitted range and plots every species.",
    "[{kind}] 직경별 탄소저장량": "[{kind}] Carbon storage by diameter",
    "설명변수": "Predictor",
    "연도별 추정이 가능한 항목이 없습니다.\n"
    "그래프 기준을 [직경별]로 바꾸면 전체 항목을 볼 수 있습니다.":
        "No entry supports a year-based projection.\n"
        "Switch the graph basis to [By diameter] to plot every entry.",
    "직경별에서는 합계 곡선을 제공하지 않습니다":
        "No summed curve on the diameter basis",
    "성장차 없는 {n}개 항목은 연도별에서 제외 (직경별로 확인)":
        "{n} entries without growth increments are omitted here (use the diameter basis)",
    "{n}개 항목은 유효범위 미제공 — 입력값 주변만 표시":
        "{n} entries have no fitted range — plotted around the entered value only",
    "{species}: 연도별 성장차가 없어 3D 시각화에서 제외됨":
        "{species}: omitted from the 3D view (no annual growth increments)",

    # ── 라이브러리 출처 안내 (핵심 22종 / 확장 55종) ──
    "핵심 라이브러리 수종 — 연도별·직경 기준 그래프를 모두 지원합니다.":
        "Core library species — both the year and diameter graph bases are available.",
    "확장 라이브러리 수종 — 연도별 성장차가 없어 직경 기준 그래프만 "
    "지원하며, 3D 시각화에는 포함되지 않습니다.":
        "Extension library species — without annual growth increments only the "
        "diameter basis is available, and it is not included in the 3D view.",
    "제공 없음 — 직경 기준 그래프만 가능":
        "Not provided — diameter basis only",
    "식 문자열 기반 (계수 편집 불가)":
        "Defined by an equation string (coefficients are not editable)",
    "유효범위 미제공 (출처에 정의역 없음)":
        "No fitted range (the source gives no domain)",
    "핵심 라이브러리 {n}건 · {c:,.2f} kgC":
        "Core library: {n} entries · {c:,.2f} kgC",
    "핵심 {cn}건 {cc:,.2f} kgC  +  확장 {en}건 {ec:,.2f} kgC  =  {t:,.2f} kgC":
        "Core {cn} entries {cc:,.2f} kgC  +  extension {en} entries {ec:,.2f} kgC "
        " =  {t:,.2f} kgC",
